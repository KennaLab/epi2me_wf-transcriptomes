...
This repository is a fork of the ONT epi2me-labs wf-transcriptomes workflow.
It contains UMCU specific changes to be able to run the wf-transcriptomes workflow on the UMCU compute environment.

## Run wf-transcriptomes on UMCU HPC

1. Modify the singularity cacheDir in [conf/umcu_hpc.config](conf/umcu_hpc.config)
2. Modify the [run sbatch script](run_epi2me_wf-transcriptomes.sh) to run the wf-human-variation workflow on the UMCU HPC. Make sure to replace all the placeholders indicated by `<>`. (At least variables `workflow_path` and `softwaretool_path`)
3. Modify following command to submit the run script. Make sure to replace all the placeholders indicated by `<>`.

    ```bash
    sh </path/to/repository/dir/>run_epi2me_wf-transcriptomes.sh \
        </path/to/input_fastq> \
        </path/to/input_samplesheet> \
        </path/to/output_dir> \
        <email> \
        --minimap2_index_opts '-k 15' \
        --ref_annotation '</path/to/annotation.gtf>' \
...
git clone git@github.com:KennaLab/epi2me_wf-transcriptomes.git
cd epi2me_wf-transcriptomes
git remote add upstream git@github.com:epi2me-labs/wf-transcriptomes.git
